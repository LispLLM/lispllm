...
export default function TraceInspector() {
  ...
  const renderTree = (node: Ast, depth: number): React.ReactNode => {
    const source = nodeSource(img.program, node).replace(/\n\s*/g, ' ');
    ...
    return (
      <div key={node.id} role="treeitem" aria-selected={selectedNodeId === node.id}>
        <button
        ...
        {children.length > 0 && (
          <div role="group">{children.map((child) => renderTree(child, depth + 1))}</div>
        )}
      </div>
    );
  };
  ...
  return (
    <section className="flex h-full min-h-0 flex-col" data-testid="s7-inspector">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto py-1" role="tree" aria-label="model trace">
        {img.program.forms.map((form) => renderTree(form, 0))}
      </div>
